"""
Test DS3M Univariate on unified electricity data.

This script tests DS3M using ONLY the target variable's history as input,
without any exogenous features. This helps assess how much predictive
power comes from the temporal patterns vs. the additional features.

Usage:
    python3 test_ds3m_unified_univariate.py --dataset DE
    python3 test_ds3m_unified_univariate.py --dataset FR
    python3 test_ds3m_unified_univariate.py --dataset DE_FR  # Spread prediction
"""
...
def prepare_univariate_data(
    ...
    """
    Prepare univariate data for DS3M training.

    Uses ONLY the target variable's history as input features.
    ...
    Returns:
        Dictionary with univariate data tensors
    """
    ...
    # Create windows for univariate
    # Input X[t] = Y[t-timestep:t] (shifted Y)
    # Target Y[t] = Y[t] (current value to predict)
    def create_univariate_windows(Y_norm, timestep):
        n_samples = len(Y_norm) - timestep
        X_win = np.zeros((n_samples, timestep, 1), dtype=np.float32)
        Y_win = np.zeros((n_samples, timestep, 1), dtype=np.float32)

        for i in range(n_samples):
            # Input: past timestep values
            X_win[i, :, 0] = Y_norm[i:i + timestep]
            # Target: next timestep values (shifted by 1)
            Y_win[i, :, 0] = Y_norm[i + 1:i + timestep + 1]
...
def train_ds3m_univariate(
    ...
    """Train DS3M univariate model."""
    device = torch.device(device)

    # Model dimensions (univariate: x_dim=1, y_dim=1)
    x_dim = 1
    y_dim = 1
...
def main():
    parser = argparse.ArgumentParser(description='Test DS3M Univariate on unified electricity data')
    parser.add_argument('--dataset', type=str, default='DE', choices=['DE', 'FR', 'DE_FR'],
                        help='Dataset to use (DE_FR for spread prediction)')
    ...
    print("=" * 70)
    print("DS3M UNIVARIATE Test on Unified Electricity Data")
    print("=" * 70)
    print(f"Dataset: {args.dataset}")
    print(f"Mode: UNIVARIATE (target history only)")
    print(f"Timestep: {args.timestep}")

    # Load data
    print("\n--- Loading Data ---")
    data = prepare_univariate_data(
        country=args.dataset,
        timestep=args.timestep
    )

    print(f"Target: {data['target_col']}")
    print(f"Train X: {data['trainX'].shape}")  # (timestep, n_samples, 1)
    ...
    if not DS3M_AVAILABLE:
        ...
        print("To install DS3M, ensure the path is correct:")
        print(f"  {DS3M_PATH}")
        return
    ...
    print(f"  DS3M Univariate: Spearman={metrics['spearman']:.4f}, RMSE={metrics['rmse']:.2f}")
    ...
    # Save results
    save_dir = Path(__file__).parent / "outputs" / f"test_ds3m_univariate_{args.dataset}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if metrics['spearman'] > baselines['persistence']['spearman']:
        print("SUCCESS: DS3M Univariate beats persistence baseline!")
    else:
        print("NOTE: DS3M Univariate did not beat persistence baseline.")
    print("=" * 70)
