"""
Test DS3M on unified electricity data.

This script verifies that DS3M can train and produce meaningful predictions
using the unified dataset (ENTSO-E + weather + calendar + outages + commodities).

Usage: python3 test_ds3m_unified.py --dataset DE
"""
...
from unified_data_loader import prepare_unified_ds3m_data, FEATURE_GROUPS
...
def compute_baseline_predictions(Y_train: np.ndarray, Y_test: np.ndarray) -> dict:
    ...
    Baselines:
    1. Persistence: predict yesterday's change
    2. Mean: predict training mean
    3. Zero: predict zero change
    """
    n_test = len(Y_test)
...
def train_ds3m(
    ...
    """
    Train DS3M model on unified electricity data.
    """
...
def main():
    parser = argparse.ArgumentParser(description='Test DS3M on unified electricity data')
    ...
    print("=" * 70)
    print("DS3M Test on Unified Electricity Data")
    print("=" * 70)
    ...
    # Save results
    save_dir = Path(__file__).parent / "outputs" / f"test_ds3m_{args.dataset}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if metrics['spearman'] > baselines['persistence']['spearman']:
        print("SUCCESS: DS3M beats persistence baseline!")
    else:
