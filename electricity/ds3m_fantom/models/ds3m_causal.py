...
class DS3MCausal(nn.Module):
    ...
    def __init__(
        ...
        self.dag_dist = SharedRegimeDAG(
            ...
            init_logits=init_logits or [0.0, 0.0],  # Balanced init (~33% edge prob), let training decide
        )
        ...
        self.causal_emissions = nn.ModuleList([
            CausalEmission(
                num_nodes=self.num_nodes,
                device=device,
                target_idx=-1,  # Predict last variable (TARGET)
                lag=lag,
                h_dim=h_dim,
                z_dim=z_dim,
                embedding_size=32,
    ...
    def forward(
        ...
        for t in range(timestep):
            ...
            for d in range(self.d_dim):
                ...
                y_pred_d, y_std_d = self.causal_emissions[d](
    ...
    def get_regime_assignments(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            # Split into x (all but last feature) and y (last feature)
            x = X_seq[:, :, :-1]  # [lag+1, T, x_dim]
            y = X_seq[:, :, -1:]  # [lag+1, T, 1]
