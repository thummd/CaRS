...
class SharedRegimeDAG(nn.Module):
    ...
    def get_dag_with_penalty(
        ...
        """
        Get adjacency matrix and compute hierarchical DAG penalty.

        For shared_backbone mode, applies DAG constraint to BOTH the shared
        and regime-specific components separately. This is critical because:
        1. The combined matrix can have very high dagness even if components are sparse
        2. Gradient flows cleanly to each component's parameters
        3. Each component being a DAG ensures the union is also acyclic
        ...
        Returns:
            ...
            dag_penalty: Hierarchical DAG penalty (sum of component penalties)
        """
        if self.sharing_mode == "shared_backbone":
            # Get both components
            if sample:
            ...
            # DAG penalty on EACH component separately
            # This is the key fix: penalize each component's dagness individually
            # so gradients flow cleanly and both converge to DAGs
            dag_penalty_shared = dagness_factor(A_shared)
            dag_penalty_regime = dagness_factor(A_regime)
            dag_penalty = dag_penalty_shared + dag_penalty_regime
